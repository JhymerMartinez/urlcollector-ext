...
var viewTabUrl = chrome.extension.getURL('urls.build.html');
...
var newMarkers = Array.prototype.slice.call(Polymer.dom(this.$.markers).getDistributedNodes());
...
this._objectsMutationObserver.observe(this, { childList: true });
},
_updateObjects: function () {
var newObjects = Array.prototype.slice.call(Polymer.dom(this.$.objects).getDistributedNodes());
...
this.map.setOptions({ disableMapTypeControl: this.disableMapTypeControl });
},
...
this.$.selector.selectedValues = this.$.selector.selectedValues.filter(function (i) {
return i !== markerIndex;
});
}
}
});
...
'bubble': { ctor: 'BubbleChart' },
'candlestick': { ctor: 'CandlestickChart' },
'column': { ctor: 'ColumnChart' },
'combo': { ctor: 'ComboChart' },
'geo': { ctor: 'GeoChart' },
'histogram': { ctor: 'Histogram' },
'line': { ctor: 'LineChart' },
'pie': { ctor: 'PieChart' },
'scatter': { ctor: 'ScatterChart' },
'stepped-area': { ctor: 'SteppedAreaChart' },
'table': {
ctor: 'Table',
pkg: 'table'
},
...
return this._loadPackages([chartData.pkg || DEFACTO_CHART_PACKAGE]).then(function () {
...
var adder = opt_once ? viz.events.addOneTimeListener : viz.events.addListener;
...
this.$.loader.create(this.type, this.$.chartdiv).then(function (chart) {
...
this.$.loader.dataTable().then(function (dataTable) {
cols.forEach(function (col) {
dataTable.addColumn(col);
});
dataTable.addRows(rows);
return dataTable;
}.bind(this)).then(this.$.loader.dataView.bind(this.$.loader)).then(function (dataView) {
...
dataPromise.then(this.$.loader.dataTable.bind(this.$.loader)).then(this.$.loader.dataView.bind(this.$.loader)).then(function (dataView) {
this._dataView = dataView;
}.bind(this));
}
});
